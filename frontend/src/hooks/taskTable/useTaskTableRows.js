...
export default function useTaskTableRows(api, {
  ...
  const prevShowFussRef = useRef(showFuss);
  ...
  useEffect(() => {
    if (prevShowFussRef.current === showFuss) return;
    prevShowFussRef.current = showFuss;
    setPage(0);
  }, [showFuss, setPage]);

  useEffect(() => {
    if (prevPickupModeRef.current === pickupMode) return;
