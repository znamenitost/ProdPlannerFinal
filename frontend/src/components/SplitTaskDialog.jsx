...
export default function TaskRow({ 
  ...
  // Полный путь для открытия файла
      ...
      <TableRow sx={getRowStyle()}>
        ...
        <TableCell>
          <Tooltip title={task.folderPath || ''} arrow>
            <Typography variant="body2" sx={{ fontSize: '0.75rem', color: '#4b5563', wordBreak: 'break-all' }}>
              {truncate(task.folderPath || '', 40)}
            </Typography>
          </Tooltip>
        </TableCell>
        <TableCell>
        ...
        <TableCell>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
            ...
            >
              <Typography variant="body2" sx={{ color: 'text.secondary', fontSize: '0.8rem', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: '200px', cursor: 'default' }}>
                {truncate(task.comment || '—', 30)}
              </Typography>
            </Tooltip>
        ...
        <TableCell align="center">
          <Typography variant="body2" sx={{ fontSize: '0.8rem' }}>{task.estimateHours.toFixed(1)} ч</Typography>
        </TableCell>
        <TableCell>
          <Tooltip title={task.type} arrow>
            <Chip label={truncate(task.type, 20)} size="small" variant="outlined" sx={{ fontSize: '0.7rem', maxWidth: '150px' }} />
          </Tooltip>
        </TableCell>
        <TableCell>
          <Chip icon={<Person sx={{ fontSize: 14 }} />} label={task.employeeName} size="small" variant="outlined" sx={{ fontSize: '0.7rem' }} />
        </TableCell>
        <TableCell>
          <Chip icon={displayStatusIcon} label={task.statusText || "Назначена"} size="small" sx={{ bgcolor: displayStatusColor, color: 'white', fontSize: '0.7rem' }} />
        </TableCell>
        <TableCell>
  ...
  return (
    <Fragment>
      <TableRow sx={getRowStyle()}>
        ...
        <TableCell>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
            <Tooltip title={task.comment || 'Нет комментария'} arrow placement="top"
              slotProps={{ tooltip: { sx: { bgcolor: '#1e293b', fontSize: '12px', padding: '8px 15px', maxWidth: '400px', borderRadius: 2 } } }}
            >
              <Typography variant="body2" sx={{ color: 'text.secondary', fontSize: '0.8rem', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: '250px', cursor: 'default' }}>
        ...
        <TableCell>
          {hasChildren ? (
            <Chip icon={displayStatusIcon} label={displayStatus} size="small" sx={{ bgcolor: displayStatusColor, color: 'white', fontSize: '0.7rem' }} />
          ) : (
            <Chip icon={displayStatusIcon} label={task.statusText || "Назначена"} size="small" sx={{ bgcolor: displayStatusColor, color: 'white', fontSize: '0.7rem' }} />
          )}
        </TableCell>
